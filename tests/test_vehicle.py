...
def test_vehicle_format(fake):
    template = "banana {Year} apple ({Make})"
    formattedString = fake.vehicle_format(template)
    assert len(formattedString.split(" ")) >= 4
    for word in template.split(" "):
        if not word.find("{"):
            assert formattedString.find("word")

def test_vehicle_make(fake, makes):
    make = fake.vehicle_make()
    assert len(make) > 1
    assert make in makes

def test_vehicle_year(fake, years):
    year = fake.vehicle_year()
    assert len(year) > 1
    assert int(year) in years

def test_vehicle_model(fake, models):
    model = fake.vehicle_model()
    assert len(model) >= 1
    assert model in models

def test_vehicle_category(fake, categories):
